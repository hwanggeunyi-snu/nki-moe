...
def nki_rmsnorm_kernel(input_tensor, weight, eps):
    """NKI RMSNorm kernel used by the submission-side local model variant."""
    max_partitions = 128
    ...
    num_chunks = (num_rows + max_partitions - 1) // max_partitions

    # Load the RMSNorm weight once into SBUF and reuse it across row tiles.
    ...
    for chunk_idx in nl.affine_range(num_chunks):
        row_start = chunk_idx * max_partitions
        valid_rows = min(max_partitions, num_rows - row_start)

        a = nl.ndarray((max_partitions, hidden_size), dtype=input_tensor.dtype, buffer=nl.sbuf)
        nisa.dma_copy(
            dst=a[0:valid_rows, 0:hidden_size],
            src=input_tensor[row_start : row_start + valid_rows, 0:hidden_size],
        )

        t = nl.ndarray((max_partitions, hidden_size), dtype=input_tensor.dtype, buffer=nl.sbuf)
        nisa.tensor_tensor(dst=t, data1=a, data2=a, op=nl.multiply)

        sq_sum = nl.ndarray((max_partitions, 1), dtype=nl.float32, buffer=nl.psum)
        nisa.tensor_reduce(dst=sq_sum, data=t, op=nl.add, axis=1)

        s = nl.ndarray((max_partitions, 1), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_scalar(
            dst=s,
            data=sq_sum,
            op0=nl.multiply,
        ...
        g_bcast = nl.ndarray((max_partitions, hidden_size), dtype=g_tile.dtype, buffer=nl.sbuf)
        stream_shuffle_broadcast(g_tile, g_bcast)
        nisa.tensor_tensor(dst=t, data1=t, data2=g_bcast, op=nl.multiply)

        nisa.dma_copy(
            dst=output[row_start : row_start + valid_rows, 0:hidden_size],
...
class Qwen3MoeInferenceConfig(InferenceConfig):
    def __init__(self, *args, **kwargs):
        ...
        # The fused MoE token-generation kernel expects router matmul inputs/weights to
        # share dtype. Keep the baseline FP32 router for the flat path, but align the
        # baked-in fused path with the model dtype so the kernel can compile.
        if getattr(self.neuron_config, "moe_fused_nki_kernel_enabled", False):
            self.neuron_config.router_config.dtype = self.neuron_config.torch_dtype
        else:
